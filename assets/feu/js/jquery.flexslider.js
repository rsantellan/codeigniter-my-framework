  ...
  $.flexslider = function(el, options) {
    ...
    methods = {
      init: function() {
		...
		slider.pauseOnAction = vars.pauseOnAction;
        // TOUCH/USECSS:
        ...
        if (vars.slideshow) {
          if (vars.pauseOnHover) {
			...
			// [++] modification: For homepage slider the hover event is applied to the whole #home-header section
			if (slider.is('#home-slider')) {
				if (!touch) {
      ...
      touch: function() {
        ...
        function onTouchStart(e) {
          if (slider.animating) {
            ...
            //slider.pause(); // [++] modification: no pausing on "touch start" event
            // CAROUSEL: 
    ...
    slider.flexAnimate = function(target, pause, override, withSync, fromNav) {
      ...
      if (!slider.animating && (slider.canAdvance(target, fromNav) || override) && slider.is(":visible")) {
		...
		// [++] modification: reset slideshow counter
		if (vars.slideshow && !vars.pauseOnAction && (!vars.pauseOnHover || touch)) {
			clearInterval(slider.animatedSlides);
			slider.animatedSlides = setInterval(slider.animateSlides, vars.slideshowSpeed);
		}
		var d = new Date(),
			utc = parseInt((d.getTime() - 1389028500000) / 1000);
		//console.log('utc='+utc+' int='+(utc-prev_utc));
		prev_utc = utc;
		// end modification
        ...
        if (!fade) {
          ...
          if (!touch) {
			...
			if ($('html').hasClass('ie8')) { // [++] modification: IE8 hack
  ...
  $.fn.flexslider = function(options) {
    ...
    if (typeof options === "object") {
      return this.each(function() {
		...
		// [++] modification: individual styling for captions and positioning caption blocks
		//      (Note: not used for the homepage slider with right-aligned image)
		if (!$this.is('#home-slider') || $this.is('#home-hero-content.full-width-image #home-slider')) {
			...
			$slides.each(function(i) { // each slide
			
				$slides.eq(i).find('.flex-caption').each(function() { // each caption block of a slide
				
					var $flexCaption = $(this),
						bgColor = $flexCaption.attr('data-bg-color'),
						bgOpacity = $flexCaption.attr('data-bg-opacity'),
						borderColor = $flexCaption.attr('data-border-color'),
						left = $flexCaption.attr('data-left'),
						right = $flexCaption.attr('data-right'),
						top = $flexCaption.attr('data-top'),
						bottom = $flexCaption.attr('data-bottom'),
						xCentered = $flexCaption.attr('data-x-centered'),
						yCentered = $flexCaption.attr('data-y-centered'),
					...
					if (borderColor) {
						...
						$flexCaption.css({'border-left': '4px solid #' + borderColor});
					}
					
					// caption position
					if (typeof left != 'undefined') { $flexCaption.css({'left': left, 'right': 'auto'}); }
					if (typeof right != 'undefined') { $flexCaption.css({'left': 'auto', 'right': right}); }
					if (typeof top != 'undefined') { $flexCaption.css({'top': top, 'bottom': 'auto'}); }
					if (typeof bottom != 'undefined') { $flexCaption.css({'top': 'auto', 'bottom': bottom}); }
					
					// caption width and height
					if (width) { $flexCaption.css({'width': width}); }
					if (height) { $flexCaption.css({'height': height}); }
					
					// caption centering (horizontal/vertical)
					if (xCentered == 'yes') {
						$flexCaption.css({'left': '0', 'right': '0', 'margin-left': 'auto', 'margin-right': 'auto'});
					}
					if (yCentered == 'yes') {
						$flexCaption.css({'top': '50%', 'bottom': 'auto', 'margin-top': -parseInt($flexCaption.outerHeight()/2)});
					}
      ...
      switch (options) {
        ...
        default: if (typeof options === "number") $slider.flexAnimate(options, $slider.pauseOnAction);
      }
    }
  }  
