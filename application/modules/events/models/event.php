...
class event extends MY_Model {
  ...
  public function getTranslation($id, $lang, $returnObject = true) {
    ...
    $this->db->limit('1');
    $query = $this->db->get('event_translation');
    if ($query->num_rows() == 1) {
      $obj = $query->row();
      if ($returnObject) {
  ...
  private function saveNew($id = null) {
    ...
    $dataTranslation = array(
        ...
        'slug' => $this->createSlug('slug', $this->getName(), 'event_translation', $id, 'lang', $this->getLang()),
    );
  ...
  private function edit() {
    $data = array(
        'name' => $this->getName(),
        'slug' => $this->createSlug('slug', $this->getName(), 'event_translation', $this->getId(), 'lang', $this->getLang()),
        'description' => $this->getDescription(),
        'members' => $this->getMembers(),
    );
    $this->db->where('id', $this->getId());
    $this->db->where('lang', $this->getLang());
    $this->db->update('event_translation', $data);
