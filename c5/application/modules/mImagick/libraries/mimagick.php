...
class mimagick {
   ...
   * @author Rodrigo Santellan
   */
  public function thumbnailWithBackground($file, $cacheFile, $width, $height, $color = imagickColors::BLACK, $extension = imagickExtensions::JPG) {
    $im = new Imagick($file);
   ...
   * @author Rodrigo Santellan
   */
  public function resizeAndCreteReflection($file, $cacheFile, $width = 200, $opacity = 0.3, $color = imagickColors::BLACK, $extension = imagickExtensions::PNG) {
    $im = new Imagick($file);
   ...
   * @author Rodrigo Santellan
   */
  public function thumbnailWithRoundCorners($file, $cacheFile, $width = 100, $height = NULL, $shadowColor = imagickColors::BLACK)
  {
    $im = new Imagick($file);
    if(is_null($height))
    {
   ...
   * @author Rodrigo Santellan
   */
  public function polaroidImage($file, $cacheFile, $color = imagickColors::LIGHTBLUE)
  {
    $im = new Imagick($file);
   ...
   * @author Rodrigo Santellan
   */
  public function basicThumbnail($file, $cacheFile, $type = imagickThumbnailTypes::NORMAL, $width = 200, $height = 200)
  {
    if(!imagickThumbnailTypes::isValidtype($type))
    {
      throw new Exception("Invalid thumbnail type check class imagickThumbnailTypes");
    }
    $im = new Imagick($file);
    switch($type)
    {
      case imagickThumbnailTypes::NORMAL:
        $im->thumbnailImage( $width, null );
        break;
      case imagickThumbnailTypes::FIT:
        $im->thumbnailImage( $width, $height, true );
        break;
      case imagickThumbnailTypes::FIXED:
        $im->thumbnailImage( $width, $height);
        break;
      case imagickThumbnailTypes::CROP:
