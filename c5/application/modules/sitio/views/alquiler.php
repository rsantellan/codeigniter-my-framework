  ...
  <?php foreach ($apartamentos_list as $apartamento): ?>
    ...
    <div class="alquileres-div float_left">
  	...
  	<div class="alquileres-info float_left">
		...
		<?php
		$precio = "Consultar";
		if ($apartamento->precio_alquiler > 0) {
		  $precio = ($apartamento->moneda_alquiler == 0) ? "U\$S" : "\$";
		  $precio .= number_format($apartamento->precio_alquiler, 0, ",", ".");
		}
		?>
  	  <span class="rojo">Valor:  <?php echo $precio; ?></span>
  	  <div class="clear"></div>
		<?php $url_help = $apartamento->id . "/" . url_title($apartamento->titulo, '-', TRUE) . ".html"; ?>
  	  <a class="boton-detalle" href="<?php echo site_url('alquiler/' . $url_help); ?>">Detalles</a>
  	</div>
    </div>
  <?php endforeach; ?>
  <div class="clear"></div>
